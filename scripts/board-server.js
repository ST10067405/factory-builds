...
const VAULT = 'C:/Users/jaime/Desktop/obsidian-vault';
...
function buildState() {
  ...
  return { projects, documents: data.documents || [], preflight: preflight(), generatedAt: new Date().toISOString() };
}

// ---------- safe path check ----------
function insideAllowed(p) {
  const rp = path.resolve(p);
  return rp.startsWith(ROOT) || rp.startsWith(path.resolve(VAULT));
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (p.startsWith('/api/project/') && req.method === 'POST') {
    ...
    const allowed = ['title', 'status', 'priority', 'notes', 'nextAction', 'caveats', 'notionUrl', 'docs', 'tags'];
    const data = loadData();
